...
# define d_m3RunCodeArgs(RUNTIME, SSP) d_m3BaseOpDefaultArgs, 0., d_m3DefaultEidRegister((RUNTIME), (SSP))
#else
# define M3_SHADOW_STACK_SLOT_SCALE 2
# define M3_SHADOW_MEMORY_SCALE     2
# define d_m3RunCodeArgs(RUNTIME, SSP) d_m3OpDefaultArgs
#endif
...
IM3Runtime  m3_NewRuntime  (IM3Environment i_environment, u32 i_stackSizeInBytes, void * i_userdata)
{
    ...
    if (runtime)
    {
        ...
        u32 shadow_stack_bytes = ((i_stackSizeInBytes / sizeof (m3slot_t)) + 4) * M3_SHADOW_STACK_SLOT_SCALE * sizeof (m3slot_t);
        void *shadow_stack = m3_Malloc ("Shadow Stack", shadow_stack_bytes); // TODO: more precise stack checks
#if defined(USE_EID)
        u32 num_register_slots = (i_stackSizeInBytes / sizeof (m3slot_t)) + 4;
        eid *eid_registers = m3_Malloc ("EID Registers", num_register_slots * sizeof (eid));
#endif

        if (runtime->originStack && shadow_stack
#if defined(USE_EID)
            && eid_registers
#endif
        )
        {
            ...
            runtime->s_stack = shadow_stack;
            runtime->numStackSlots = i_stackSizeInBytes / sizeof (m3slot_t);         m3log (runtime, "new stack: %p", runtime->originStack);
#if defined(USE_EID)
            runtime->eid_registers = eid_registers;
            memset (runtime->eid_registers, 0x0, num_register_slots * sizeof (eid));
#endif
        }
        else
        {
            m3_Free (runtime->originStack);
            m3_Free (shadow_stack);
#if defined(USE_EID)
            m3_Free (eid_registers);
#endif
            m3_Free (runtime);
            runtime = NULL;
        }
...
void  Runtime_Release  (IM3Runtime i_runtime)
{
    ...
    m3_Free (i_runtime->eid_registers);
...
M3Result  EvaluateExpression  (IM3Module i_module, void * o_expressed, u8 i_type, bytes_t * io_bytes, cbytes_t i_end)
{
    ...
    runtime.s_stack = i_module->runtime->s_stack;

    m3stack_t stack = (m3stack_t)runtime.stack;
    m3stack_t s_stack = (m3stack_t)runtime.s_stack;

    IM3Runtime savedRuntime = i_module->runtime;
    M3MemoryHeader *memory = savedRuntime ? savedRuntime->memory.mallocated : NULL;
    M3MemoryHeader *s_memory = savedRuntime ? savedRuntime->s_memory.mallocated : NULL;
...
# if (d_m3EnableOpProfiling || d_m3EnableOpTracing)
            m3ret_t r = RunCode (m3code, stack, s_stack, memory, s_memory, d_m3RunCodeArgs (savedRuntime, s_stack), d_m3BaseCstr);
# else
            m3ret_t r = RunCode (m3code, stack, s_stack, memory, s_memory, d_m3RunCodeArgs (savedRuntime, s_stack));
# endif
...
M3Result  InitMemory  (IM3Runtime io_runtime, IM3Module i_module)
{
    ...
    if (not i_module->memoryImported)
    {
        ...
        io_runtime->s_memory.maxPages = M3_SHADOW_MEMORY_SCALE * io_runtime->memory.maxPages;
        io_runtime->s_memory.pageSize = pageSize ? pageSize : d_m3DefaultMemPageSize; // page size is same for shadow memory
...
M3Result  ResizeMemory  (IM3Runtime io_runtime, u32 i_numPages)
{
    ...
    u32 numPagesToAlloc_s = i_numPages * M3_SHADOW_MEMORY_SCALE;

    M3Memory * memory = & io_runtime->memory;
    M3Memory * s_memory = & io_runtime->s_memory;

    if ((numPagesToAlloc <= memory->maxPages) && (numPagesToAlloc_s <= s_memory->maxPages))
    {
        size_t numPageBytes = numPagesToAlloc * io_runtime->memory.pageSize;
        size_t numPageBytes_s = numPagesToAlloc_s * io_runtime->memory.pageSize;
        ...
        size_t numBytes_s = numPageBytes_s + sizeof (M3MemoryHeader);

        size_t numPreviousBytes = memory->numPages * io_runtime->memory.pageSize;
        if (numPreviousBytes)
            numPreviousBytes += sizeof (M3MemoryHeader);

        size_t numPreviousBytes_s = s_memory->numPages * io_runtime->memory.pageSize;
        if (numPreviousBytes_s)
            numPreviousBytes_s += sizeof (M3MemoryHeader);

        void* newMem = m3_Realloc ("Wasm Linear Memory", memory->mallocated, numBytes, numPreviousBytes);
        _throwifnull(newMem);
        void* newMem_s = m3_Realloc ("Wasm Shadow Memory", s_memory->mallocated, numBytes_s, numPreviousBytes_s);
        _throwifnull(newMem_s);

        memory->mallocated = (M3MemoryHeader*)newMem;
        s_memory->mallocated = (M3MemoryHeader*)newMem_s;
...
# endif
        ...
        s_memory->numPages = numPagesToAlloc_s;

        memory->mallocated->length =  numPageBytes;
        memory->mallocated->runtime = io_runtime;

        s_memory->mallocated->length =  numPageBytes_s;
        s_memory->mallocated->runtime = io_runtime;

        memory->mallocated->maxStack = (m3slot_t *) io_runtime->stack + io_runtime->numStackSlots;
        s_memory->mallocated->maxStack = (m3slot_t *) io_runtime->s_stack + (M3_SHADOW_STACK_SLOT_SCALE * io_runtime->numStackSlots);
...
# if (d_m3EnableOpProfiling || d_m3EnableOpTracing)
        result = (M3Result) RunCode (function->compiled, (m3stack_t) runtime->stack, (m3stack_t) runtime->s_stack, runtime->memory.mallocated, runtime->s_memory.mallocated, d_m3RunCodeArgs (runtime, (m3stack_t) runtime->s_stack), d_m3BaseCstr);
# else
        result = (M3Result) RunCode (function->compiled, (m3stack_t) runtime->stack, (m3stack_t) runtime->s_stack, runtime->memory.mallocated, runtime->s_memory.mallocated, d_m3RunCodeArgs (runtime, (m3stack_t) runtime->s_stack));
# endif
...
M3Result  m3_CallVL  (IM3Function i_function, va_list i_args)
{
    ...
    if (getenv("WASM3_EOP_LOG") != NULL) { WASM3_EOP_LOG = true; }
    if (getenv("WASM3_DEBUG_LOG") != NULL) { WASM3_DEBUG_LOG = true; }
    if (getenv("WASM3_FULL_LOG") != NULL) { WASM3_FULL_LOG = true; }
    WASM3_FP_TIMING = (getenv("WASM3_FP_TIMING") != NULL);
    if (getenv("SECOND_ORDER_ERR") != NULL) { SECOND_ORDER_ERR = true; }
    const char *ct = getenv("COND_THRESH");
    if (ct) {
        COND_THRESH = atoi(ct);
        printf("Condition number threshold: %d\n", COND_THRESH);
    }
    const char *ll = getenv("LOG_LIMIT");
    if (ll) {
        LOG_LIMIT = atoi(ll);
        printf("Log limit: %d\n", LOG_LIMIT);
    }

    const char *result_dir = getenv("RESULT_DIR");
    if (!result_dir) {
        fprintf(stderr, "RESULT_DIR is not set\n");
        return NULL;
    }

#if d_m3HasErrorSlot
    char debug_path[256];
    snprintf(debug_path, sizeof(debug_path), "%s/%s", result_dir, "DEBUG-ID.txt");
    load_debug_ids(debug_path);

    char log_fn_path[256];
    snprintf(log_fn_path, sizeof(log_fn_path), "%s/%s", result_dir, "LOG-FN-ID.txt");
    load_log_fn_ids(log_fn_path);

    char log_fp_path[256];
    snprintf(log_fp_path, sizeof(log_fp_path), "%s/%s", result_dir, "LOG-FP-ID.txt");
    load_log_fp_ids(log_fp_path);
#endif

#if defined(USE_MPFR) || defined(USE_MPFR_DD)

    int prec_bits = PREC;
    const char *prec_str = getenv("MPFR_PREC");
    if (prec_str) {
        prec_bits = atoi(prec_str);
    }
    mpfr_set_default_prec(prec_bits);
    printf("Using MPFR with precision: %d\n", prec_bits);

#elif defined(USE_EID)

    char override_path[256];
    snprintf(override_path, sizeof(override_path), "%s/%s", result_dir, "OVERRIDE.bin");
    load_override_table(override_path);

    char silent_path[256];
    snprintf(silent_path, sizeof(silent_path), "%s/%s", result_dir, "SILENT-ID.txt");
    load_silent_ids(silent_path);
    sort_silent_ids();
    
    char probe_path[256];
    snprintf(probe_path, sizeof(probe_path), "%s/%s", result_dir, "PROBE-ID.txt");
    load_probe_ids(probe_path);
    sort_probe_ids();

    char tmp[256];
    snprintf(tmp, sizeof tmp, "%s/%s", result_dir, temp_override_fn);
    temp_override_fp = tmp;
    temp_override_file = fopen(temp_override_fp, "ab");
...
# endif
    ...
    reset_fp_cycle_counter();

# if (d_m3EnableOpProfiling || d_m3EnableOpTracing)
    result = (M3Result) RunCode (i_function->compiled, (m3stack_t)(runtime->stack), (m3stack_t)(runtime->s_stack), runtime->memory.mallocated, runtime->s_memory.mallocated, d_m3RunCodeArgs (runtime, (m3stack_t) runtime->s_stack), d_m3BaseCstr);
# else
    result = (M3Result) RunCode (i_function->compiled, (m3stack_t)(runtime->stack), (m3stack_t)(runtime->s_stack), runtime->memory.mallocated, runtime->s_memory.mallocated, d_m3RunCodeArgs (runtime, (m3stack_t) runtime->s_stack));
# endif
...
# if d_m3HasFloat
        case c_m3Type_f32:  
            *va_arg(o_rets, f32*) = *(f32*)(s);  s += 8; break;
        case c_m3Type_f64:  
            *va_arg(o_rets, f64*) = *(f64*)(s);  s += 8; break;
# endif
        default: return "unknown argument type";
        }
    }
