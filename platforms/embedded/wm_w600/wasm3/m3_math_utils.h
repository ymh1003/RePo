...
#if d_m3HasErrorSlot
#define OP_TRUNC_new(RES, A_V, A_E, TYPE, RMIN, RMAX)       \
    if (M3_UNLIKELY(isnan(A_V))) {                          \
        newTrap (m3Err_trapIntegerConversion);              \
    }                                                       \
    if (M3_UNLIKELY(A_V <= RMIN or A_V >= RMAX)) {          \
        newTrap (m3Err_trapIntegerOverflow);                \
    }                                                       \
    RES = (TYPE)A_V;
#endif
...
#if d_m3HasErrorSlot
#define OP_I32_TRUNC_F32_new(RES, A_V, A_E)    OP_TRUNC_new(RES, A_V, A_E, i32, -2147483904.0f, 2147483648.0f)
#define OP_U32_TRUNC_F32_new(RES, A_V, A_E)    OP_TRUNC_new(RES, A_V, A_E, u32,          -1.0f, 4294967296.0f)
#define OP_I32_TRUNC_F64_new(RES, A_V, A_E)    OP_TRUNC_new(RES, A_V, A_E, i32, -2147483649.0 , 2147483648.0 )
#define OP_U32_TRUNC_F64_new(RES, A_V, A_E)    OP_TRUNC_new(RES, A_V, A_E, u32,          -1.0 , 4294967296.0 )

#define OP_I64_TRUNC_F32_new(RES, A_V, A_E)    OP_TRUNC_new(RES, A_V, A_E, i64, -9223373136366403584.0f,  9223372036854775808.0f)
#define OP_U64_TRUNC_F32_new(RES, A_V, A_E)    OP_TRUNC_new(RES, A_V, A_E, u64,                   -1.0f, 18446744073709551616.0f)
#define OP_I64_TRUNC_F64_new(RES, A_V, A_E)    OP_TRUNC_new(RES, A_V, A_E, i64, -9223372036854777856.0 ,  9223372036854775808.0 )
#define OP_U64_TRUNC_F64_new(RES, A_V, A_E)    OP_TRUNC_new(RES, A_V, A_E, u64,                   -1.0 , 18446744073709551616.0 )
#else
...
#if d_m3HasErrorSlot
#define OP_TRUNC_SAT_new(RES, A_V, A_E, TYPE, RMIN, RMAX, IMIN, IMAX)            \
    if (M3_UNLIKELY(isnan(A_V))) {                          \
        RES = 0;                                            \
    } else if (M3_UNLIKELY(A_V <= RMIN)) {                  \
        RES = IMIN;                                         \
    } else if (M3_UNLIKELY(A_V >= RMAX)) {                  \
        RES = IMAX;                                         \
    } else {                                                \
        RES = (TYPE)A_V;                                    \
    }
...
#if d_m3HasErrorSlot
#define OP_I32_TRUNC_SAT_F32_new(RES, A_V, A_E)    OP_TRUNC_SAT_new(RES, A_V, A_E, i32, -2147483904.0f, 2147483648.0f,   INT32_MIN,  INT32_MAX)
#define OP_U32_TRUNC_SAT_F32_new(RES, A_V, A_E)    OP_TRUNC_SAT_new(RES, A_V, A_E, u32,          -1.0f, 4294967296.0f,         0UL, UINT32_MAX)
#define OP_I32_TRUNC_SAT_F64_new(RES, A_V, A_E)    OP_TRUNC_SAT_new(RES, A_V, A_E, i32, -2147483649.0 , 2147483648.0,    INT32_MIN,  INT32_MAX)
#define OP_U32_TRUNC_SAT_F64_new(RES, A_V, A_E)    OP_TRUNC_SAT_new(RES, A_V, A_E, u32,          -1.0 , 4294967296.0,          0UL, UINT32_MAX)

#define OP_I64_TRUNC_SAT_F32_new(RES, A_V, A_E)    OP_TRUNC_SAT_new(RES, A_V, A_E, i64, -9223373136366403584.0f,  9223372036854775808.0f, INT64_MIN,  INT64_MAX)
#define OP_U64_TRUNC_SAT_F32_new(RES, A_V, A_E)    OP_TRUNC_SAT_new(RES, A_V, A_E, u64,                   -1.0f, 18446744073709551616.0f,      0ULL, UINT64_MAX)
#define OP_I64_TRUNC_SAT_F64_new(RES, A_V, A_E)    OP_TRUNC_SAT_new(RES, A_V, A_E, i64, -9223372036854777856.0 ,  9223372036854775808.0,  INT64_MIN,  INT64_MAX)
#define OP_U64_TRUNC_SAT_F64_new(RES, A_V, A_E)    OP_TRUNC_SAT_new(RES, A_V, A_E, u64,                   -1.0 , 18446744073709551616.0,       0ULL, UINT64_MAX)
#else
